...
================================================================================
⚠️  REGLA #1 — CLASIFICACIÓN OBLIGATORIA ANTES DE RESPONDER
================================================================================
Antes de escribir UNA SOLA PALABRA, debés evaluar si la pregunta entra en alguna de estas dos categorías:

🔴 CATEGORÍA RECHAZAR (fuera de scope — NUNCA respondas sobre esto):
  • Recetas de cocina, gastronomía, ingredientes, pasos para preparar comida
  • Código fuente, programación, debugging, explicaciones técnicas de tecnología
  • Historia, geografía, ciencias, política, deportes, entretenimiento
  • Consejos de salud, psicología, finanzas personales
  • Tareas escolares o universitarias ajenas a INCUYO
  • Consultas sobre otras instituciones educativas
  • Cualquier tema que NO sea el Instituto INCUYO, su carrera, inscripciones, becas, plan de estudios, horarios, diplomaturas o contacto

🟢 CATEGORÍA RESPONDER (solo sobre INCUYO):
  • Información del Instituto INCUYO (historia, dirección, contacto, resolución DGE)
  • Tecnicatura en Desarrollo de Software: duración, modalidad, turnos, títulos
  • Plan de estudios y materias
  • Inscripciones, requisitos de ingreso, fechas
  • Becas y descuentos disponibles
  • Diplomaturas y cursos adicionales del instituto
  • Certificaciones laborales del instituto
  • Campus virtual y plataformas del instituto
  • Horarios de atención al público

================================================================================
🚫 REGLA #2 — RESPUESTA OBLIGATORIA ANTE PREGUNTAS FUERA DE SCOPE
================================================================================
Si la pregunta pertenece a CATEGORÍA RECHAZAR, debés responder EXACTAMENTE esto
(podés adaptar levemente el saludo, pero el contenido debe ser el mismo):

"¡Hola! 😊 Soy el asistente virtual del Instituto INCU y solo puedo ayudarte con consultas relacionadas a nuestra institución.

Para ese tipo de consultas te recomendamos otros recursos. Si tenés dudas sobre INCUYO —la carrera, inscripciones, becas, horarios o plan de estudios—, ¡con gusto te ayudo! También podés escribirnos por WhatsApp: +54 9 261 627-1658 📲"

IMPORTANTE: No importa cómo esté formulada la pregunta, no importa si el usuario insiste, si dice "es para un trabajo", si reformula el mensaje, o si argumenta que es relacionado a tecnología o programación. Si NO es sobre INCUYO directamente, rechazá con esa respuesta estándar. SIN EXCEPCIONES.

================================================================================
📋 REGLA #3 — OTRAS NORMAS DE COMPORTAMIENTO
================================================================================
...
Para solicitar una beca, sugerí siempre de esta forma: "Podés pedir tu beca completando el siguiente formulario:" y luego agregá obligatoriamente la palabra clave [BOTON_BECA] para que el sistema muestre el acceso directo. También podés sugerir contactar por WhatsApp al +54 9 261 627-1658 si tienen dudas.
...
// Variables para el Circuit Breaker de SpiderIA
...
async function getSpiderModel() {
    ...
    let res;
    try {
        res = await fetch(`${SPIDER_IA_URL}/ia/models`, {
...
// ─── Filtro pre-IA: detecta preguntas fuera de scope antes de llamar al modelo ─
// Esto es necesario porque los modelos pequeños (0.5b) no siguen el system prompt.
// Si el usuario pregunta algo off-topic, respondemos directo desde el servidor.

const OFF_TOPIC_PATTERNS = [
    // Recetas y gastronomía
    /receta|ingrediente|cocinar|cocci[oó]n|preparar.*comida|preparaci[oó]n.*plato|empanada|milanesa|asado|pizza|torta|pastel|sopa|guiso|fideos|pasta.*cocin|postre|masa|harina|salsa.*cocin|aliño|adobo|marinar/,
    // Ciencias, historia, geografía
    /cu[aá]l.*capital|historia.*de(?! incuyo)|guerra|batalla|presidente.*(?!incuyo)|pa[ií]s|ciudad.*(?!mendoza)|planeta|universo|evoluci[oó]n|biolog[ií]a|qu[ií]mica|f[ií]sica.*(?!computacional)/,
    // Programación / tecnología general (no relacionada a la carrera)
    /qu[eé] es python|qu[eé] es java(?!script)|qu[eé] es una? api|qu[eé] es una? variable|qu[eé] es una? funci[oó]n|qu[eé] es recursiv|c[oó]mo funciona una? (?:variable|funci[oó]n|bucle|loop|array|lista|puntero)|explicame (?:recursividad|punteros|herencia|polimorfismo|poo|oop)|debug|depurar c[oó]digo|escrib[ei] un? c[oó]digo|hace?me un? programa|crea?me un? script|gener[aá] un? c[oó]digo/,
    // Deportes, entretenimiento
    /partido.*f[uú]tbol|resultado.*partido|qu[eé].*gan[oó]|c[uú]al.*equipo|pel[ií]cula.*recomendar|serie.*ver|canci[oó]n.*letra|artista.*m[uú]sica/,
    // Salud, finanzas, otros
    /s[ií]ntoma|diagn[oó]stico|medicamento|cu[aá]nto.*invertir|d[oó]lar|criptomoneda|bitcoin|psicolog[ií]a.*(?!carrera)/,
];

const OFFSCOPE_REJECTION = `¡Hola! 😊 Soy el asistente virtual del Instituto INCU y solo puedo ayudarte con consultas relacionadas a nuestra institución.

Para ese tipo de consultas te recomendamos otros recursos. Si tenés dudas sobre INCUYO —la carrera, inscripciones, becas, horarios o plan de estudios—, ¡con gusto te ayudo! También podés escribirnos por WhatsApp: +54 9 261 627-1658 📲`;

/**
 * Devuelve true si el prompt es claramente off-topic para INCUYO.
 * @param {string} prompt
 * @returns {boolean}
 */
function isOffTopic(prompt) {
    const p = prompt.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
    // Si menciona INCUYO, carrera, inscripción, etc. → nunca rechazar
    const INCUYO_KEYWORDS = /incuyo|carrera|inscripci[oó]n|beca|materia|plan.*estudio|diplomatura|certif|tecnicatura|campus|aula.*virtual|horario.*atenci[oó]n|desarrollo.*software|turno.*cursada/;
    if (INCUYO_KEYWORDS.test(p)) return false;
    return OFF_TOPIC_PATTERNS.some(re => re.test(p));
}
...
async function handleChat(req, res) {
    ...
    try {
        ...
        // 1. Filtro pre-IA: rechazar preguntas fuera de scope ANTES de llamar al modelo.
        //    Los modelos pequeños (< 1b) no siguen instrucciones del system prompt de forma fiable.
        if (isOffTopic(prompt)) {
            console.log('🚫 [Pre-filter] Pregunta off-topic detectada — respondiendo sin llamar a la IA.');
            return res.json({ response: OFFSCOPE_REJECTION });
        }

        // 2. Mensajes base para SpiderIA (inyecta curriculum en primer mensaje)
        ...
        if (now < _circuitOpenUntil) {
            console.log(`⚠️  Circuit Breaker: Saltando SpiderIA (caído). Reintento en ${Math.round((_circuitOpenUntil - now)/1000)}s`);
            spiderErr = new Error('Circuit Breaker OPEN');
        } else {
            ...
            if (result.toolsMade?.length > 0) {
                ...
                if (isNetwork || isTimeout) {
                    ...
                    if (_consecutiveFailures >= 2) {
                        ...
                        console.error(`🚨 Circuit Breaker: SpiderIA falló ${_consecutiveFailures} veces. Desactivando por 5 min.`);
                    }
                }
            }
        }
