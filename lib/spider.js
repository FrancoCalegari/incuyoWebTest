...
async function uploadFile(fileBuffer, filename, mimetype) {
    ...
    // If it's a Buffer, set knownLength. If it's a stream, it calculates automatically
    if (Buffer.isBuffer(fileBuffer)) {
        options.knownLength = fileBuffer.length;
    }
    form.append('files', fileBuffer, options);
    ...
    return {
        id: file.id,
        url: file.url || `${API_URL}/storage/files/${file.id}?api_key=${API_KEY}`,
